...
function resetResultsView() {
  ...
  renderKeyValueList(els.weakSequences, {});
...
function currentErrorCount() {
  return state.errorEvents.length;
}

function resetRecommendationsView(message = "Aucune recommandation chargee.") {
...
function renderRecommendationsList(recommendations = []) {
  ...
  recommendations.forEach((recommendation, index) => {
    ...
    titleBlock.innerHTML = `
      <p class="section-label">Suggestion ${index + 1}</p>
    ...
    head.append(titleBlock, launchButton);
    ...
    (recommendation.focus_labels || []).forEach((label) => {
...
async function loadRecommendations() {
  ...
  if (!userName) {
    alert("Renseigne un nom pour charger l'entrainement cible.");
    els.userName.focus();
    return;
  }

  try {
    const payload = await fetchJson(`${API_BASE_URL}/recommendations/${encodeURIComponent(userName)}`);

    state.recommendations = payload.recommendations || [];
    els.recommendationsStatus.textContent = payload.message || "Recommandations chargees.";
    renderRecommendationsSummary(payload.weakness_summary || {});
...
async function startRecommendedSession(recommendation) {
  ...
  if (!userName) {
    alert("Renseigne un nom avant de lancer un entrainement cible.");
    els.userName.focus();
    return;
  }
  ...
  try {
    ...
    loadSessionIntoPlayer(session);
  } catch (error) {
    console.error(error);
    alert("Impossible de lancer l'entrainement cible.");
  }
}

function getEventTimestampMs() {
  if (!state.sessionStartedAt) {
    return 0;
  }

  return Math.max(Date.now() - state.sessionStartedAt.getTime(), 0);
}
...
function recordKeyEvent({
  ...
  state.keyEvents.push({
    ...
    position,
    timestamp_ms: getEventTimestampMs(),
...
async function loadDetailedAnalysis(sessionId) {
  try {
    ...
    renderKeyValueList(els.weakSequences, payload.weak_sequences || payload.weak_bigrams || {});
...
async function startPracticeSeries() {
  try {
    ...
    if (!userName) {
      alert("Renseigne un nom avant de lancer une serie.");
...
function bindEvents() {
  ...
  els.userName.addEventListener("input", () => {
    if (!els.userName.value.trim()) {
      resetRecommendationsView("Renseigne ton nom puis charge tes recommandations personnalisees.");
    }
  });
