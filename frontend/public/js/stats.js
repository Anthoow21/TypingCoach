...
function getHeatmapCaption() {
  if (state.heatmapMode === "errors") {
    return "Chaque touche affiche le nombre d'erreurs associees.";
  }
  ...
  return "Chaque touche affiche le nombre de frappes validees.";
}
...
function renderKeyboardHeatmap() {
  ...
  const metrics = KEYBOARD_LAYOUT
    .flat()
    .map((key) => getHeatmapMetric(stats[key]))
  ...
  KEYBOARD_LAYOUT.forEach((row) => {
    ...
    row.forEach((key) => {
      ...
      keyEl.dataset.mode = state.heatmapMode;
      ...
      value.textContent = metric
        ? state.heatmapMode === "latency"
          ? `${formatNumber(metric)} ms`
          : String(metric)
        : "-";
...
async function loadStats() {
  ...
  try {
    ...
    console.error(err);
    resetStatsView();
    if (!String(err.message || "").includes("No sessions found")) {
      alert(err.message || "Erreur chargement stats");
    }
  }
}
...
async function resetUserStats() {
  ...
  if (!user) {
    alert("Entre un nom a reinitialiser.");
    return;
  }

  const confirmed = window.confirm(`Supprimer toutes les stats de ${user} ?`);
  if (!confirmed) {
    return;
  }
  ...
  try {
    ...
    alert("Les stats ont ete reinitialisees.");
  } catch (error) {
    console.error(error);
    alert("Erreur lors de la reinitialisation des stats.");
  }
}
